...
export class FriendsService {
  ...
  async updateFriendStatus(updateFriendDto: UpdateFriendDto) {
    const relationShip = await this.findRelationship(updateFriendDto.friendId, updateFriendDto.userId);
  ...
  async deleteFriend(deleteFriendDto: DeleteFriendDto) {
		const relationShip = await this.findRelationship(deleteFriendDto.friendId, deleteFriendDto.userId);
		if (!relationShip) {
			return relationShip;
		}
